...
+ (void)RegistApp{
    ...
    [WXApi registerApp:APP_ID universalLink:[[ZDPay_OrderSureModel sharedSingleten] getModelData].associate_domain];
}
...
- (void)zdpay_handleOpenURL:(NSNotification *)noti {
    [WXApi handleOpenURL:[noti.userInfo objectForKey:@"url"] delegate:self];
}
...
- (void)PayWithPrograms:(NSDictionary *)programs
    ...
    req.package            = [programs objectForKey:@"packageValue"];
    req.sign               = [programs objectForKey:@"paySign"];
    //wx53a612d04b9e1a22://pay/?returnKey=&ret=0
