...
//  Agent
//
//  Created by ideasforHK on 2018/9/11.
...
- (void)createView{
    ...
    _darkView.alpha = 0.6;
    _darkView.backgroundColor = RGB(46, 49, 50);
    
    _darkView.frame = [UIScreen mainScreen].bounds;
    ...
    // 容器视图
    _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, KScreenHeight, KScreenWidth, 225)];
    ...
    // 提示文字
    _tipLabel = [[UILabel alloc] initWithFrame:CGRectMake(80, 0, KScreenWidth-160, 44)];
    ...
    // 确定按钮
    UIButton *ctBtn = [[UIButton alloc] initWithFrame:CGRectMake(KScreenWidth-80, 0, 80, 44)];;
    ctBtn.backgroundColor = [UIColor clearColor];
    [ctBtn setTitleColor:KBlueColor forState:UIControlStateNormal];
    ...
    // 分割线
    UIView *spliteView = [[UIView alloc] initWithFrame:CGRectMake(0, 44, KScreenWidth, 1)];
    spliteView.backgroundColor = UIColorFromRGB(0xf6f6f6);
    [_contentView addSubview:spliteView];
    
    // 选择器
    _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 45, KScreenWidth, 200)];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    for(UIView *singleLine in pickerView.subviews)
    {
        ...
        {
            singleLine.backgroundColor = RGBA(0, 0, 0, 0.3);
...
//            str = [_dataArray[row] componentsSeparatedByString:@"-"].firstObject;
           ...
           NSDictionary *dict = _dataArray[row];
            ...
            str = [NSString stringWithFormat:@"%@ %@",dict[@"country"],dict[@"areaCode"]];
...
- (void)cancelBtnClicked{
    [UIView animateWithDuration:0.3f
                     animations:^{
                         self->_contentView.y = KScreenHeight;
                     }
...
- (void)show{
    ...
    [UIView animateWithDuration:0.5f animations:^{
        self->_contentView.y = KScreenHeight-225;
//        self->_contentView.centerY = self->_darkView.centerY;
    }];
}
